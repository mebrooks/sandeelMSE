...
library(smsR)
years_Rec_above_Blim = years_Rec_above_Blim[years_Rec_above_Blim %in% subset(getR(df.tmb, sas), SE<0.5)$years]
...
## size of MSE ----
nyears=2 #years to simulate forward
ntrials=2
Fcap=0.5#seq(0.21, 0.31, by=0.01)
TACmax= 1000000
TACmin= 0 #c(0, 5000)
...
#df.tmb$Surveyobs[,"2021", "RTM"]*0.001
#						0         1         2         3         4 
RTMpast = c(-1, 24728.818, 26167.961, 2076.158, -1) 
...
triallist = parApply(cl, grid, 1, function(grid){
	...
	for(yr in 1:nyears) {
		...
		#organize bio pars for forecast according to stock annex vals in forecast_stock_settings.R 
		...
		#check what Fbar corresponds to the suggested TAC given the estimated state
		# and seasonal effort
		Forecast$Fbar = calc.fterm(target.TAC= TACsuggested, E=Fest[,,yr], N=Nestmat, 
															 M=M.for, PM=PM.for, west=west.for, weca=weca.for, Fcap= Fhist, R=R.for)$minimum
		
		#save what the HCR expected
		...
		# and seasonal effort
		Fbar = calc.fterm(target.TAC= TACsuggested, E=Etrue, N=Ntrue1, 
		...
		### RTM-------
		if(RTM){
			df.assess$Surveyobs[,df.assess$nyears-1,"RTM"] = RTMpast #from last year
			RTMpast = df.assess$Surveyobs[,df.assess$nyears,"RTM"] #save to fill in next year
			df.assess$Surveyobs[,df.assess$nyears,"RTM"] = rep(-1, length(df.assess$age))
			df.assess$Surveyobs[,df.assess$nyears-11,"RTM"] = rep(-1, length(df.assess$age))
		}
	} ### end yearly for-loop ---------------------------------------------------
