...
wd <- 'M:/Tobis/Tobis_assessment/SMS_2022_Benchmark/SAN_area_3r_Current/test'
...
# reps <- sas$reps
#
# sms.admb <- read.table(file.path(wd,'summary_table_raw.out'), header = TRUE)
#
#
# df.plot.tmb <- data.frame(SSB = as.numeric(reps$value[names(reps$value) == "SSB"][1:df.tmb$nyears]),
#                      R = as.numeric(reps$value[names(reps$value) == "Rsave"][1:df.tmb$nyears]),
#                      Catchtot = as.numeric(reps$value[names(reps$value) == 'Catchtot']),
#                      model = 'TMB',
#                      year = df.tmb$years
#
# )
