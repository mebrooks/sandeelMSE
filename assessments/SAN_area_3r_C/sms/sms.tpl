...
DATA_SECTION
 ...
 // Normalise to mean effort=1   SKAL 鷹DRES TIL NORMALISERES INDENFOR 霹R鬢KE MED SAMME EXPLOITATION PATTERN
 number sum_effort
 !! if (any_do_effort==1) {
...
!! for (y=fyData;y<=lyData+2;y++) for (q=fq;q<=lq;q++) {
 ...
 // !!  for (s=first_VPA;s<=nsp;s++) {cout<<"CANUM: s: "<<s<<endl; for (y=fyData;y<=lyData;y++) cout<<y<<endl<<obs_C_input(s,y)<<endl;  }
...
//lower and upper value for stomach variance parameter 
 ...
 // change to sdreport if uncertanites for forecast are used
 ...
 // change to likeprof if uncertanites for forecast are used
 // likeprof_number SSB_likeprof;
 number SSB_likeprof;
 !! SSB_likeprof=0;
 
 // change to likeprof if uncertanites for forecast are used
 number stock_N_likeprof;
 ...
 //sdreport_matrix M2_sd0(1,nprey,fyModel,lyModel)
 ...
 //sdreport_matrix M2_sd2(1,nprey,fyModel,lyModel)
