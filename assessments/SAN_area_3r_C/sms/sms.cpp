...
model_data::model_data(int argc,char * argv[]) : ad_comm(argc,argv)
{
 ...
 cout<<"SMS version September 2014 using ADMB version 10.1"<<endl;
 _strdate(dateStr);
 _strtime(f_timeStr);
 time(&start_time);
 int par_file=0;
 ...
 for (i=1;i<argc;i++) {
  ...
  test_output.allocate("test_output");
 if (test_output==1) cout<<"test.output: "<<test_output<<endl;
  OP_output.allocate("OP_output");
 ...
 for (q=fq;q<recq;q++) faq(q)=fa+1;
  max_a.allocate("max_a");
 if (test_output==1) cout<<"max.age: "<<max_a<<endl;
  tmp.allocate(1,nsp,1,11,"tmp");
 if (test_output==1) cout<<"species.info:"<<endl<<tmp<<endl;
  la.allocate(1,nsp);
  nplus.allocate(1,nsp);
  is_pred.allocate(1,nsp);
  is_prey.allocate(1,nsp);
  do_effort.allocate(1,nsp);
 any_do_effort=0;
 all_do_effort=0;
 nOthPred=0;
 npr=0;
 ...
 first_VPA=nOthPred+1;
  F_y_species_index.allocate(first_VPA,nsp);
  Rec_add_inf.allocate(first_VPA,nsp,1,2);
 ...
 if (npr==0) npr=1;
  ...
  nprey=sum(is_prey);
  SSB_Rec_model.allocate(first_VPA,nsp);
  use_beta_SSB_Rec.allocate(first_VPA,nsp);
  SSB_Rec_hockey_breakpoint.allocate(first_VPA,nsp);
 read_temperature=0;
 for (s=first_VPA;s<=nsp;s++) {
  ...
  la_pred.allocate(1,npr);
 for (s=1;s<=npr;s++) la_pred(s)=la(s);
  la_VPA.allocate(first_VPA,nsp);
  las.allocate(first_VPA,nsp);
  cfa.allocate(first_VPA,nsp);
  la_like.allocate(first_VPA,nsp);
 first_VPA_prey=0;
 for (s=first_VPA;s<=nsp;s++) {
   la_VPA(s)=la(s);
  ...
  use_known_rec_option.allocate("use_known_rec_option");
 if (test_output==1) cout<<"known.recruitment: "<<use_known_rec_option<<endl;
  SSB_R_beta_cor.allocate(first_VPA,nsp,"SSB_R_beta_cor");
 if (test_output==1) cout<<"SSB.R.beta.cor:"<<endl<<SSB_R_beta_cor<<endl;
  SSB_year_first.allocate(first_VPA,nsp,"SSB_year_first");
  SSB_year_last.allocate(first_VPA,nsp,"SSB_year_last");
 for (s=first_VPA;s<=nsp;s++) {
 ...
 if (test_output==1) cout <<"combined.catches: "<<seasonal_annual_catches<<endl;
  seasonal_combined_catch_s2.allocate(first_VPA,nsp,"seasonal_combined_catch_s2");
 ...
 for (s=first_VPA;s<=nsp;s++) {
  ...
  n_catch_s2_group.allocate(first_VPA,nsp,"n_catch_s2_group");
 if (test_output==1) cout<<"n.catch.s2.group: "<<n_catch_s2_group<<endl;
  catch_s2_group.allocate(first_VPA,nsp,1,n_catch_s2_group,"catch_s2_group");
 if (test_output==1) cout<<"catch.s2.group: "<<endl<<catch_s2_group<<endl;
  n_catch_season_age_group.allocate(first_VPA,nsp,"n_catch_season_age_group");
 if (test_output==1) cout <<"n.catch.season.age.group: "<<endl<<n_catch_season_age_group<<endl;
  catch_season_age.allocate(first_VPA,nsp,1,n_catch_season_age_group,"catch_season_age");
 if (test_output==1) cout<<"catch.season.age: "<<endl<<catch_season_age<<endl;
  avg_F_ages.allocate(first_VPA,nsp,1,2,"avg_F_ages");
 if (test_output==1) cout<<"avg.F.ages: "<<endl<<avg_F_ages<<endl;
  min_catch.allocate(first_VPA,nsp,"min_catch");
 if (test_output==1) cout<<"min.catch: "<<endl<<min_catch<<endl;
  n_catch_sep_year_group.allocate(first_VPA,nsp,"n_catch_sep_year_group");
 if (test_output==1) cout<<"n.catch.sep.year.group: "<<endl<<n_catch_sep_year_group<<endl;
  catch_sep_year.allocate(first_VPA,nsp,1,n_catch_sep_year_group,"catch_sep_year");
 ...
 for (s=first_VPA;s<=nsp;s++) for (i=1;i<=n_catch_sep_year_group(s);i++) { 
  ...
  no_F_y_groups.allocate(first_VPA,nsp);
 for (s=first_VPA;s<=nsp;s++) {
   no_F_y_groups(s)=0;
 ...
 for (s=first_VPA;s<=nsp;s++) {
  ...
  Nknots.allocate(first_VPA,nsp,"Nknots");
 if (test_output==1) cout<<"Nknots: "<<endl<<Nknots<<endl;
  knotsX.allocate(first_VPA,nsp,1,Nknots,"knotsX");
 if (test_output==1) cout<<"Years for nodes: "<<endl<<knotsX<<endl;
  zero_catch_year_season.allocate("zero_catch_year_season");
 ...
 if  (zero_catch_year_season!=1 && zero_catch_year_season!=0) {
  ...
  zero_catch_season_age.allocate("zero_catch_season_age");
 ...
 if  (zero_catch_season_age!=1 && zero_catch_season_age!=0) {
  ...
  fix_F_factor.allocate(first_VPA,nsp,"fix_F_factor");
 if (test_output==1) cout<<"fix.F.factor:"<<endl<< fix_F_factor<<endl;
  est_calc_sigma.allocate(1,3,"est_calc_sigma");
 ...
 if (est_calc_sigma(1)==0) est_catch_sigma=1; else est_catch_sigma=0;
  read_HCR_file.allocate("read_HCR_file");
 ...
 if (multi>0 && !(size_select_model>=1 && size_select_model<=6)) {
  ...
  L50_mesh.allocate(first_VPA,nsp,"L50_mesh");
 if (test_output==1 && multi>0) cout <<"L50_mesh: "<<L50_mesh<<endl;
  size_selection.allocate(1,npr,"size_selection");
 ...
 if (test_output==1 && multi>0) cout <<"var.scale.stom: "<<endl<<setprecision(3)<<Stom_var_fac<<endl;
  size_other_food_suit.allocate(1,npr,"size_other_food_suit");
 ...
 if (multi==1) def_pred_par=1;  else def_pred_par=0;
  phase_vulnera.allocate("phase_vulnera");
 if (test_output==1 && multi>0) cout<<"phase.vulnera: "<<phase_vulnera<<endl; 
  phase_stl_other_suit_slope.allocate("phase_stl_other_suit_slope");
 if (test_output==1 && multi>0) cout<<"phase.other.suit.slope: "<<phase_stl_other_suit_slope<<endl;
  phase_pref_size_ratio.allocate("phase_pref_size_ratio");
 ...
 if (use_size_selection==0) phase_pref_size_ratio=-1;
  phase_pref_size_ratio_correction.allocate("phase_pref_size_ratio_correction");
 ...
 if (use_size_selection==0) phase_pref_size_ratio_correction=-1;
  phase_prey_size_adjustment.allocate("phase_prey_size_adjustment");
 ...
 if (use_size_selection==0) phase_prey_size_adjustment=-1;
  phase_var_size_ratio.allocate("phase_var_size_ratio");
 ...
 if (do_short_term_forecast>0) ad_comm::change_datafile_name("short-term-configuration.dat");
  ...
  no_F_multipliers.allocate("no_F_multipliers");
  F_multipliers.allocate(first_VPA,nsp,1,no_F_multipliers,"F_multipliers");
  rec_TAC_year.allocate(first_VPA,nsp,"rec_TAC_year");
 if (zero_catch_year_season==1) ad_comm::change_datafile_name("zero_catch_year_season.in");
 else ad_comm::change_datafile_name("just_one.in");
  zero_catch_y_season.allocate(first_VPA,nsp,fyData,lyData,fq,lq,"zero_catch_y_season");
 ...
 if (zero_catch_season_age==1) ad_comm::change_datafile_name("zero_catch_season_ages.in");
  ...
  incl_catch_season_age.allocate(first_VPA,nsp,fq,lq,fa,max_a,"incl_catch_season_age");
 ...
 if (test_output==2 && zero_catch_season_age==1) cout<<"incl_catch_season_age(s,q,a):"<<endl<<incl_catch_season_age<<endl;
  fq_F.allocate(first_VPA,nsp,cfa,la_like);
  lq_F.allocate(first_VPA,nsp,cfa,la_like);
 ...
 for (s=first_VPA;s<=nsp;s++) for (a=cfa(s);a<=la_like(s);a++) for (q=fq;q<=lq;q++)
  ...
  lcq.allocate(1,no_sp_sag_syg);
  fcq.allocate(1,no_sp_sag_syg);
 int sp_sag_syg=0;
 no_sp_sag=0;
 max_syg=1; max_sag=1;
 for (s=first_VPA;s<=nsp;s++) {
 ...
 if (seasonal_annual_catches_any==1) ad_comm::change_datafile_name("f_q_ini.in"); 
  ...
  F_q_ini_read.allocate(1,no_sp_sag_syg,fcq,lcq,"F_q_ini_read");
 ...
 ad_comm::change_datafile_name("recruitment_years.in"); 
  recruitment_years.allocate(first_VPA,nsp,fyData,lyData,"recruitment_years");
 if (test_output>=1) cout<<"recruitment years:"<< endl<<recruitment_years<<endl;
 save_SSB_Rec_residuals=0;
  SSB_Rec_nobs.allocate(first_VPA,nsp);
  SSB_Rec_residuals.allocate(first_VPA,nsp,1,ny_model);
 if (OP_output==1) {pnsp=nsp; pnpr=npr; pnopr=nOthPred; ad_comm::change_datafile_name("op.dat");}
  ...
  OP_dummy.allocate(1,6,"OP_dummy");
 ...
 if (OP_do_growth_all>1) OP_do_growth_all=1;
  OP_dummy_parms.allocate(1,6,first_VPA,pnsp,"OP_dummy_parms");
  OP_dummy1.allocate("OP_dummy1");
  OP_dummy2.allocate("OP_dummy2");
  OP_other_N.allocate(1,2,1,pnopr,"OP_other_N");
 ...
 if (OP_output==1) {ad_comm::change_datafile_name("reference_points.in");}
  ...
  reference_points.allocate(1,nsp,1,4,"reference_points");
 ...
 if (do_growth_all>1) do_growth_all=1;
  last_year_season_F_adjustment.allocate(first_VPA,pnsp,"last_year_season_F_adjustment");
 if (read_HCR_file==1) if (test_output==51) cout <<"last.Seasons.adjustment.factor:"<< last_year_season_F_adjustment <<endl;
  stochastic_recruitment.allocate(first_VPA,pnsp,"stochastic_recruitment");
 if (read_HCR_file==1) if (test_output==51) cout <<"stochastic.recruitment:"<<endl<<stochastic_recruitment<<endl;
  rec_noise_trunc_a.allocate(first_VPA,pnsp,"rec_noise_trunc_a");
  rec_noise_trunc_b.allocate(first_VPA,pnsp,"rec_noise_trunc_b");
  rec_noise_trunc.allocate(first_VPA,pnsp,1,2);
 ...
 if (read_HCR_file==1) if (test_output==51) cout <<"rec.noise.trunc:"<<endl<<rec_noise_trunc<<endl;  
  rec_noise_input.allocate(first_VPA,pnsp,"rec_noise_input");
 if (read_HCR_file==1) if (test_output==51) cout <<"rec.noise.input:"<< rec_noise_input <<endl;
  tmp2.allocate(1,47,first_VPA,pnsp,"tmp2");
 if (read_HCR_file==1) if (test_output==51) cout<<"hcr.input:"<<endl<<tmp2<<endl;
  HCR.allocate(first_VPA,pnsp);
  HCR_copy.allocate(first_VPA,pnsp);
  T1.allocate(first_VPA,pnsp);
  T2.allocate(first_VPA,pnsp);
  maxFT1.allocate(first_VPA,pnsp,1,2);
  maxFT1T2.allocate(first_VPA,pnsp,1,2);
  maxFT2.allocate(first_VPA,pnsp,1,2);
  HCR_F_TAC.allocate(first_VPA,pnsp);
  constantF.allocate(first_VPA,pnsp);
  constantTAC.allocate(first_VPA,pnsp);
  target_SSB.allocate(first_VPA,pnsp);
  real_time_F.allocate(first_VPA,pnsp);
  F_cap.allocate(first_VPA,pnsp);
  TAC_cap.allocate(first_VPA,pnsp);
  TAC_min.allocate(first_VPA,pnsp);
  F_constraint.allocate(first_VPA,pnsp,1,2);
  TAC_constraint.allocate(first_VPA,pnsp,1,2);
  TAC_constraint_copy.allocate(first_VPA,pnsp,1,2);
  SSB_constraint.allocate(first_VPA,pnsp,1,2);
  obs_noise_trunc.allocate(first_VPA,pnsp,1,2);
  real_time_uncertanty.allocate(first_VPA,pnsp,1,4);
  real_time_uncertanty_copy.allocate(first_VPA,pnsp,1,4);
  survey_uncertanty.allocate(first_VPA,pnsp,1,4);
  assess_uncertanty.allocate(first_VPA,pnsp,1,4);
  implemt_uncertanty.allocate(first_VPA,pnsp,1,4);
  TAC_first.allocate(first_VPA,pnsp);
  TAC_second.allocate(first_VPA,pnsp);
  F_first.allocate(first_VPA,pnsp);
  F_second.allocate(first_VPA,pnsp);
  interYear.allocate(first_VPA,pnsp);
  inter_F_TAC.allocate(first_VPA,pnsp);
 for (s=first_VPA;s<=pnsp;s++) {
     HCR(s)                  =int(tmp2(1,s));
     T1(s)                   =tmp2(2,s);
     T2(s)                   =tmp2(3,s);
 ...
 for (s=first_VPA;s<=pnsp;s++) {
  ...
  HCR_state.allocate(first_VPA,nsp);
  HCR_state=1;
  input_no_recruit_autocor.allocate(first_VPA,nsp,"input_no_recruit_autocor");
 if (read_HCR_file==1) if (test_output==51) cout<<"input_no_recruit_autocor:"<<input_no_recruit_autocor<<endl;
  no_recruit_autocor.allocate(first_VPA,nsp);
 ...
 for (s=first_VPA;s<=nsp;s++) if (input_no_recruit_autocor(s)<1) no_recruit_autocor(s)=1;
  recruit_autocor.allocate(first_VPA,nsp,1,no_recruit_autocor,"recruit_autocor");
 ...
 if (read_predict_N_first_year>lyModel) read_predict_N=1; else read_predict_N=0; 
  n_init_pop.allocate("n_init_pop");
 if (test_output==51 && read_HCR_file==1) cout<<"n.init.pop: "<<n_init_pop<<endl;
  read_expl_pat_first_year.allocate("read_expl_pat_first_year");
  read_expl_pat_last_year.allocate("read_expl_pat_last_year");
 ...
 if (read_HCR_file==0) read_expl_pat=0; 
  ...
  use_read_expl_pat.allocate("use_read_expl_pat");
 ...
 if (nOthPred==0) loc_nOthPred=1;
  ...
  other_pred_N_change.allocate(1,loc_nOthPred,"other_pred_N_change");
 for (s=1;s<=loc_nOthPred;s++) if (other_pred_N_change(s)<0) {
   cout<<"ERROR: option other_pred_N_change must be >= 0"<<endl;
    exit(9);
 }
  other_pred_N_first_year.allocate(1,loc_nOthPred,"other_pred_N_first_year");
  other_pred_N_last_year.allocate(1,loc_nOthPred,"other_pred_N_last_year");
 if (test_output==51) {
 ...
 if (read_cov==0) { ad_comm::change_datafile_name("just_one.in"); tmpA=-100;}
  ...
  coVariance.allocate(first_VPA,nsp,fa,max_a,tmpA,la_VPA,"coVariance");
 ...
 if (read_var_age==0) { ad_comm::change_datafile_name("just_one.in"); tmpA=-100;}
  ...
  assessment_CV_age.allocate(first_VPA,nsp,tmpA,la_VPA,"assessment_CV_age");
 ...
 if (read_predict_N==0) ad_comm::change_datafile_name("just_one.in");
  ...
  init_predict_N.allocate(first_VPA,nsp,read_predict_N_first_year,read_predict_N_last_year,1,n_init_pop,fa,max_a,"init_predict_N");
 ...
 if (read_expl_pat==0) ad_comm::change_datafile_name("just_one.in");
  ...
  init_exploitation_pattern.allocate(first_VPA,nsp,read_expl_pat_first_year,read_expl_pat_last_year,fq,lq,fa,max_a,"init_exploitation_pattern");
 ...
 if (do_growth_type_1==0 && OP_do_growth_type_1==0 ) {lnsp=first_VPA; ad_comm::change_datafile_name("just_one.in");}
  ...
  growth_type1_regr.allocate(first_VPA,lnsp,fa,max_a,1,3,"growth_type1_regr");
 if (do_growth_type_1==1 && test_output>=51) {
  ...
  growth_type1_in_year.allocate(first_VPA,lnsp,fa,max_a,fq,lq,"growth_type1_in_year");
 if (do_growth_type_1==1 && test_output>=51) {
  cout<<"growth_type1.in, in year increment ratio:"<<endl<<setprecision(3)<<endl<<growth_type1_in_year<<endl;
 }
  growth_ratio_weca_west.allocate(first_VPA,lnsp,fq,lq,fa,max_a,"growth_ratio_weca_west");
 if (do_growth_type_1==1 && test_output>=51) {
  cout<<"growth_type1.in, ratio between weca and west"<<endl<<setprecision(2)<<endl<<growth_ratio_weca_west<<endl;
 }
  growth_min_w.allocate(first_VPA,lnsp,fa,max_a,"growth_min_w");
 if (do_growth_type_1==1 && test_output>=51) {
  cout<<"Growth, minimum weight at age Q1"<<endl<<setprecision(2)<<endl<<growth_min_w<<endl;
 }
  growth_max_w.allocate(first_VPA,lnsp,fa,max_a,"growth_max_w");
 if (do_growth_type_1==1 && test_output>=51) {
 ...
 if (test_output==1) cout<<"Start to read file: fleet_info.dat"<<endl;
  CPUE_minStd.allocate("CPUE_minStd");
 ...
 if (test_output==1) cout<<"fleet.CPUE.CV:"<<endl<<CPUE_minStd<<endl;
  n_fleet.allocate(first_VPA,nsp,"n_fleet");
 if (test_output==1) cout<<"n.fleet:"<<endl<<n_fleet<<endl; 
  fleet_info.allocate(first_VPA,nsp,1,n_fleet,1,10,"fleet_info");
 if (test_output==1) cout<<"Fleet_info:"<<endl<<setfixed()<<setprecision(0)<<fleet_info<<endl; 
  first_fleet_year.allocate(first_VPA,nsp,1,n_fleet);
  last_fleet_year.allocate(first_VPA,nsp,1,n_fleet);
  fleet_alfa.allocate(first_VPA,nsp,1,n_fleet);
  fleet_beta.allocate(first_VPA,nsp,1,n_fleet);
 no_sp_fl=0;
 for (s=first_VPA;s<=nsp;s++) for (f=1;f<=n_fleet(s);f++){
  ...
  first_fleet_age.allocate(first_VPA,nsp,1,n_fleet);
  last_fleet_age.allocate(first_VPA,nsp,1,n_fleet);
  last_fleet_age_q.allocate(first_VPA,nsp,1,n_fleet);
  last_fleet_age_power.allocate(first_VPA,nsp,1,n_fleet);
  n_CPUE_s2_group.allocate(first_VPA,nsp,1,n_fleet);
  fleet_season.allocate(first_VPA,nsp,1,n_fleet);
  v_first_fleet_age.allocate(1,no_sp_fl);
  v_last_fleet_age_q.allocate(1,no_sp_fl);
  v_last_fleet_age.allocate(1,no_sp_fl);
  v_last_fleet_age_plus_one.allocate(1,no_sp_fl);
  v_last_fleet_age_power.allocate(1,no_sp_fl);
  v_n_CPUE_s2_group.allocate(1,no_sp_fl);
  dummy_v_n_CPUE_s2_group.allocate(1,no_sp_fl);
  v_first_fleet_year.allocate(1,no_sp_fl);
  v_last_fleet_year.allocate(1,no_sp_fl);
 int sp_fl=0;  
 for (s=first_VPA;s<=nsp;s++) for (f=1;f<=n_fleet(s);f++){
   sp_fl++;
  ...
  qq_power_index.allocate(first_VPA,nsp,1,n_fleet,first_fleet_age,last_fleet_age);
 ...
 for (s=first_VPA;s<=nsp;s++) for (f=1;f<=n_fleet(s);f++){
  ...
  CPUE_s2_group.allocate(first_VPA,nsp,1,n_fleet,1,n_CPUE_s2_group,"CPUE_s2_group");
 ...
 ad_comm::change_datafile_name("fleet_catch.in");
  fleet_catch_tmp.allocate(1,no_sp_fl,v_first_fleet_year,v_last_fleet_year,v_first_fleet_age,v_last_fleet_age_plus_one,"fleet_catch_tmp");
  fleet_catch.allocate(1,no_sp_fl,v_first_fleet_year,v_last_fleet_year,v_first_fleet_age,v_last_fleet_age);
 if (test_output==2) {
   sp_fl=0;
  ...
  fleet_effort.allocate(1,no_sp_fl,v_first_fleet_year,v_last_fleet_year);
 ...
 if (make_sim_data==1) for (s=first_VPA;s<=nsp;s++) j+= seasonal_combined_catch_s2(s)*n_catch_s2_group(s);
  catch_s2_input.allocate(1,i,1,j,"catch_s2_input");
  catch_s2_index.allocate(first_VPA,nsp,1,seasonal_combined_catch_s2,1,n_catch_s2_group);
 i=0; 
 if (make_sim_data==1) {
  ...
  qq_s2_input.allocate(1,i,1,j,"qq_s2_input");
 if (any_do_effort==0) {     // normal SMS
  ...
  effort.allocate(first_VPA,tmpE,fq,lq,fyData,lyData,"effort");
 ...
 def_yqd=yqd;
 // with area
  yqdAll.allocate(1,def_yqd);
 for (i=1;i<=def_yqd;i++) yqdAll(i)=nsp;
  yqdsAll_fa.allocate(1,def_yqd,1,nsp);
 for (y=fyData;y<=lyData;y++) for (q=fq;q<=lq;q++) for (d=1;d<=no_areas;d++)  {
  CALC_yqd 
  yqdsAll_fa(yqd)=faq(q); 
 }
  yqdsAll_la.allocate(1,def_yqd,1,nsp);
 for (i=1;i<=def_yqd;i++) for (s=1;s<=nsp;s++) yqdsAll_la(i,s)=la(s);
  yqdVPA.allocate(1,def_yqd);
 for (i=1;i<=def_yqd;i++) yqdVPA(i)=nsp;
  yqdsVPA_fa.allocate(1,def_yqd,first_VPA,nsp);
 for (y=fyData;y<=lyData;y++) for (q=fq;q<=lq;q++) for (d=1;d<=no_areas;d++)  {
  CALC_yqd 
  yqdsVPA_fa(yqd)=faq(q); 
 }
  yqdsVPA_la.allocate(1,def_yqd,first_VPA,nsp);
 for (i=1;i<=def_yqd;i++) for (s=first_VPA;s<=nsp;s++) yqdsVPA_la(i,s)=la(s);
 k=max(1,npr); 
  yqdPred.allocate(1,def_yqd);
 for (i=1;i<=def_yqd;i++) yqdPred(i)=k;
  yqdsPred_fa.allocate(1,def_yqd,1,k);
 for (y=fyData;y<=lyData;y++) for (q=fq;q<=lq;q++) for (d=1;d<=no_areas;d++)  {
  CALC_yqd 
  yqdsPred_fa(yqd)=faq(q); 
 }     
  yqdsPred_la.allocate(1,def_yqd,1,k);
 for (i=1;i<=def_yqd;i++) for (s=1;s<=k;s++) yqdsPred_la(i,s)=la(s);
 k=max(1,nOthPred);
  yqdOPred.allocate(1,def_yqd);
 for (i=1;i<=def_yqd;i++) yqdOPred(i)=k;
  yqdsOPred.allocate(1,def_yqd,1,k);
 for (i=1;i<=def_yqd;i++) for (s=1;s<=k;s++) yqdsOPred(i,s)=la(s);
// ########
 // without area
  v_last_sp.allocate(1,yq);
 v_last_sp=nsp;
  vAll_fa.allocate(1,yq,1,nsp);
 for (y=fyData;y<=lyData;y++) for (q=fq;q<=lq;q++) {
  i=(y-fyData)*lq+q;  
  vAll_fa(i)=faq(q);
 }
  vAll_la.allocate(1,yq,1,nsp);
 for (i=1;i<=yq;i++) for (s=1;s<=nsp;s++) vAll_la(i,s)=la(s);
 yq1=(lyData-fyData+1)*lq+lq;
  v_last_sp1.allocate(1,yq1);
 v_last_sp1=nsp;
  vAll_fa1.allocate(1,yq1,1,nsp);
 for (y=fyData;y<=lyData+1;y++) for (q=fq;q<=lq;q++) {
  i=(y-fyData)*lq+q;
  vAll_fa1(i)=faq(q);
 }
  vAll_la1.allocate(1,yq1,1,nsp);
 for (i=1;i<=yq1;i++) for (s=1;s<=nsp;s++) vAll_la1(i,s)=la(s);
 yq2=(lyData-fyData+2)*lq+lq;
  v_last_sp2.allocate(1,yq2);
 v_last_sp2=nsp;
  vAll_fa2.allocate(1,yq2,1,nsp);
 for (y=fyData;y<=lyData+2;y++) for (q=fq;q<=lq;q++) {
  i=(y-fyData)*lq+q;
  vAll_fa2(i)=faq(q);
 }
  vAll_la2.allocate(1,yq2,1,nsp);
 for (i=1;i<=yq2;i++) for (s=1;s<=nsp;s++) vAll_la2(i,s)=la(s);
v_last_sp=nsp;
  vVPA_fa.allocate(1,yq,first_VPA,nsp);
 for (y=fyData;y<=lyData;y++) for (q=fq;q<=lq;q++) {
  i=(y-fyData)*lq+q;
  vVPA_fa(i)=faq(q);
 }
  vVPA_la.allocate(1,yq,first_VPA,nsp);
 for (i=1;i<=yq;i++) for (s=first_VPA;s<=nsp;s++) vVPA_la(i,s)=la(s);
 yq1=(lyData-fyData+1)*lq+lq;
  vVPA_fa1.allocate(1,yq1,first_VPA,nsp);
 for (y=fyData;y<=lyData+1;y++) for (q=fq;q<=lq;q++) {
  i=(y-fyData)*lq+q;
  vVPA_fa1(i)=faq(q);
 }
  vVPA_la1.allocate(1,yq1,first_VPA,nsp);
 for (i=1;i<=yq1;i++) for (s=first_VPA;s<=nsp;s++) vVPA_la1(i,s)=la(s);
 yq2=(lyData-fyData+2)*lq+lq;
  vVPA_fa2.allocate(1,yq2,first_VPA,nsp);
 for (y=fyData;y<=lyData+2;y++) for (q=fq;q<=lq;q++) {
  i=(y-fyData)*lq+q;
  vVPA_fa2(i)=faq(q);
 }
  vVPA_la2.allocate(1,yq2,first_VPA,nsp);
 for (i=1;i<=yq2;i++) for (s=first_VPA;s<=nsp;s++) vVPA_la2(i,s)=la(s);
  v_last_Psp.allocate(1,yq);
v_last_Psp=npr;
  vpr_fa.allocate(1,yq,1,npr);
 for (y=fyData;y<=lyData;y++) for (q=fq;q<=lq;q++) {
  i=(y-fyData)*lq+q;
  vpr_fa(i)=faq(q);
 }
  vpr_la.allocate(1,yq,1,npr);
 for (i=1;i<=yq;i++) for (s=1;s<=npr;s++) vpr_la(i,s)=la(s);
  v_last_Osp.allocate(1,yq);
v_last_Osp=nOthPred;
  vOpr_fa.allocate(1,yq,1,nOthPred);
 for (y=fyData;y<=lyData;y++) for (q=fq;q<=lq;q++) {
  i=(y-fyData)*lq+q;
  vOpr_fa(i)=faq(q);
 }
  vOpr_la.allocate(1,yq,1,nOthPred);
 for (i=1;i<=yq;i++) for (s=1;s<=nOthPred;s++) vOpr_la(i,s)=la(s);
  vys1.allocate(fyData,lyData);
  vys2.allocate(fyData,lyData);
 vys1=first_VPA;
 vys2=nsp;
  vVPA_lay.allocate(fyData,lyData,first_VPA,nsp);
  vVPA_fay.allocate(fyData,lyData,first_VPA,nsp);
 for (i=fyData;i<=lyData;i++) for (s=first_VPA;s<=nsp;s++) {vVPA_fay(i,s)=fa; vVPA_lay(i,s)=la(s);}
  p_last_sp.allocate(fq,lq);
 p_last_sp=nsp;
  p_VPA_fa.allocate(fq,lq,first_VPA,nsp);
 for (q=fq;q<=lq;q++) {
  p_VPA_fa(q)=faq(q);
 }
  p_VPA_la.allocate(fq,lq,first_VPA,nsp);
 for (q=fq;q<=lq;q++) for (s=first_VPA;s<=nsp;s++) p_VPA_la(q,s)=la(s);
  p_All_fa.allocate(fq,lq,1,nsp);
 for (q=fq;q<=lq;q++) {
  p_All_fa(q)=faq(q);
 }
  p_All_la.allocate(fq,lq,1,nsp);
 ...
 if (do_short_term_forecast>0) lastyear=lyData+1; else lastyear=lyData;
  weca_input.allocate(first_VPA,nsp,fyData,lastyear,fq,lq,fa,max_a,"weca_input");
 if (test_output==2) cout<<"Weight in the catch from file weca.in:"<<endl<<weca_input<<endl;
  CW.allocate(first_VPA,nsp,fyData,lyData,recq,recq,fa,max_a);
  change_input("canum.in"); 
  obs_C_input.allocate(first_VPA,nsp,fyData,lyData,fq,lq,fa,max_a,"obs_C_input");
 ...
 // cout<<"Weight in the catch from file weca.in and after annaul catch adjustment:"<<endl<<weca_input<<endl;
  obs_C.allocate(1,yq,first_VPA,v_last_sp,vVPA_fa,vVPA_la);
 MOVE(obs_C_input,obs_C,first_VPA,nsp,lyData);
  weca.allocate(1,yq1,first_VPA,v_last_sp1,vVPA_fa1,vVPA_la1);
 MOVE(weca_input,weca,first_VPA,nsp,lastyear);
 change_input("natmor.in");
 if (do_short_term_forecast>0) lastyear=lyData+1; else lastyear=lyData;
  M_input.allocate(first_VPA,nsp,fyData,lastyear,fq,lq,fa,max_a,"M_input");
 if (test_output==2) cout<<"Natural mortalities from file natmor.in:"<<endl<<M_input<<endl;
  M.allocate(1,yq1,first_VPA,v_last_sp1,vVPA_fa1,vVPA_la1);
  M1M2.allocate(1,yq1,first_VPA,v_last_sp1,vVPA_fa1,vVPA_la1);
 ...
 if (do_short_term_forecast>0 || any_do_effort==1) lastyear=lyData+2; else lastyear=lyData;
  west_input.allocate(1,nsp,fyData,lastyear,fq,lq,fa,max_a,"west_input");
 if (test_output==2) cout<<"Weight in the stock from file west.in:"<<endl<<west_input<<endl;
  west.allocate(1,yq2,1,v_last_sp2,vAll_fa2,vAll_la2);
 ...
 if (calc_discard==0) change_input("just_one.in");
  prop_landed_input.allocate(first_VPA,nsp,fyData,lyData,fq,lq,fa,max_a,"prop_landed_input");
 if (calc_discard==0) {
 ...
 y=lyData; q=lq; CALC_yq
  prop_landed.allocate(1,yq,first_VPA,v_last_sp,vVPA_fa,vVPA_la);
 MOVE(prop_landed_input,prop_landed,first_VPA,nsp,lyData);
 change_input("propmat.in");
 if (do_short_term_forecast>0 || any_do_effort==1) lastyear=lyData+2; else lastyear=lyData;
  propmat_input.allocate(first_VPA,nsp,fyData,lastyear,fq,lq,fa,max_a,"propmat_input");
 if (test_output==2) cout<<"proportion mature from file propmat.in:"<<endl<<propmat_input<<endl;
  propmat.allocate(1,yq2,first_VPA,v_last_sp2,vVPA_fa2,vVPA_la2);
 MOVE(propmat_input,propmat,first_VPA,nsp,lastyear);
 change_input("proportion_m_and_f_before_spawning.in");
  prop_M.allocate(first_VPA,nsp,"prop_M");
  prop_F.allocate(first_VPA,nsp,"prop_F");
 ...
 if (read_temperature==0) ad_comm::change_datafile_name("just_one.in");
  ...
  temperature.allocate(fyData,lyData,"temperature");
 ...
 if (use_known_rec_option==0) ad_comm::change_datafile_name("just_one.in");
  ...
  known_recruitment.allocate(first_VPA,nsp,fyData,lyData,"known_recruitment");
 ...
 if (test_output<3 && test_output>0) cout << endl << " Input of single species input data completed" << endl;
  log_obs_C_annual.allocate(fyData,lyData,first_VPA,vys2,vVPA_fay,vVPA_lay);
 y=lyData; q=lq; CALC_yq; 
  log_obs_C.allocate(1,yq,first_VPA,v_last_sp,vVPA_fa,vVPA_la);
  sum_C.allocate(first_VPA,nsp,fq,lq,fa,la_VPA);
  sum_C_annual.allocate(first_VPA,nsp,fa,la_VPA);
 if (multi==0) ad_comm::change_datafile_name("just_one.in");
 if (multi==0) {
 ...
 if (multi==0) Mindex=-1; else Mindex=max_a;
  M1_input.allocate(first_VPA,Mnsp,fyData,Mly,fq,Mlq,fa,Mindex,"M1_input");
 if (test_output==3 && multi>0) cout<<"M1 from file natmor1.in:"<<endl;
  M1.allocate(1,yq,first_VPA,v_last_sp,vVPA_fa,vVPA_la);
 ...
 if (multi==0) Mindex=-1; else Mindex=first_VPA+nprey-1;
  size_range_a_b.allocate(1,4,1,Mnpr,first_VPA,Mindex,"size_range_a_b");
 if (multi>=1 && read_size_a_b==1 && test_output==3) {
  cout<<"predator prey size range quantiles from pred_prey_size_range_param.in"<<endl;
  cout<<setprecision(3)<<size_range_a_b(1)<<endl<<endl;
  cout<<setprecision(3)<<size_range_a_b(2)<<endl<<endl;
  cout<<setprecision(3)<<size_range_a_b(3)<<endl<<endl;
  cout<<setprecision(3)<<size_range_a_b(4)<<endl<<endl;
 }
 ...
 if (multi==0) Mindex=-1; else Mindex=nsp;
  overlap.allocate(1,no_areas,1,Mnpr,fyData,Mly,fq,Mlq,0,Mindex,"overlap");
 ...
 if (multi==0) Mindex=-1; else Mindex=nsp;
  overlap_forecast.allocate(1,no_areas,1,Mnpr,lyModel+1,lpy,fq,Mlq,0,Mindex,"overlap_forecast");
 ...
 if (multi==0) Mindex=-1; else Mindex=nsp;
  season_overlap_input.allocate(1,no_areas,1,Mnpr,fq,Mlq,0,Mindex,"season_overlap_input");
 if (test_output==3 && multi>0) cout<<"Overlap from file season_overlap.in:"<<endl<<season_overlap_input<<endl;
 if (multi==0) Mindex=-1; else Mindex=nsp;
  season_overlap_index.allocate(1,no_areas,1,Mnpr,fq,Mlq,0,Mindex);
 ...
 if (multi>=1 && no_areas>1) ad_comm::change_datafile_name("predator_area_presence.in");
  ...
  pred_area_present.allocate(1,npr,1,no_areas,"pred_area_present");
 ...
 if (multi>=1 && consum_op>=1) { ad_comm::change_datafile_name("consum_ab.in"); Mindex=lq;}
  ...
  consum_ab.allocate(1,no_areas,1,Mnpr,fq,Mindex,1,2,"consum_ab");
 ...
 if (multi>=1 && consum_op==0) { ad_comm::change_datafile_name("consum.in"); Mindex=max_a;}
  ...
  consum_input.allocate(1,no_areas,1,Mnpr,fyData,Mly,fq,Mlq,fa,Mindex,"consum_input");
 if (test_output==3 && multi>0 && consum_op==0) cout<<"Consum (ration) from file consum.in:"<<endl<<consum_input<<endl;
  consum.allocate(1,def_yqd,1,yqdPred,yqdsPred_fa,yqdsPred_la);
 ...
 if (no_areas==1) Mindex=-1; else Mindex=max_a;
  N_dist_input.allocate(1,no_areas,1,Mnsp,fyData,lyData,fq,Mlq,fa,Mindex,"N_dist_input");
 ...
 if (multi>=1) for (il=1;il<=n_ALK_yqdsa;il++) {
  ...
  ALK.allocate(1,n_ALK_yqdsa,f_ALK_yqdsal,l_ALK_yqdsal,"ALK");
 if (test_output==3 && multi>0) {
   cout<<"alk file in):"<<endl;
   for (il=1;il<=n_ALK_yqdsa;il++) {
     cout<<ALK(il)<<" SUM:"<<sum(ALK(il))<<endl;
  }}
  ALK_length.allocate(1,n_ALK_yqdsa,f_ALK_yqdsal,l_ALK_yqdsal,"ALK_length");
 ...
 if (multi>=1) for (il=1;il<=n_ALKS_yqdsa;il++) {
  ...
  ALKS_input.allocate(1,n_ALKS_yqdsa,f_ALKS_yqdsal,l_ALKS_yqdsal,"ALKS_input");
 if (test_output==3 && multi>0 && simple_ALK==1) cout<<"alk all file in):"<<endl<<ALKS_input<<endl;
  ALKS_length.allocate(1,n_ALKS_yqdsa,f_ALKS_yqdsal,l_ALKS_yqdsal,"ALKS_length");
 if (test_output==3 && multi>0 && simple_ALK==1) {
  ...
  minlAll.allocate(1,nsp,fa,la);
  maxlAll.allocate(1,nsp,fa,la);
  minlVPA.allocate(first_VPA,nsp,fa,la_VPA);
  maxlVPA.allocate(first_VPA,nsp,fa,la_VPA);
  minlPred.allocate(1,npr,fa,la_pred);
  maxlPred.allocate(1,npr,fa,la_pred);
  minmaxAll_fa.allocate(1,nsp);
  minmaxAll_la.allocate(1,nsp);
  minmaxVPA_fa.allocate(first_VPA,nsp);
  minmaxVPA_la.allocate(first_VPA,nsp);
  minmaxPred_fa.allocate(1,npr);
  minmaxPred_la.allocate(1,npr);
  minmaxAll_fa=fa;
  minmaxVPA_fa=fa;
  minmaxPred_fa=fa;
  ...
  index_Lq.allocate(fyData,lyData,fq,lq);
 if (multi==0) Mindex=-1; else Mindex=max_a;
  index_minl.allocate(1,def_yqd,1,yqdAll,yqdsAll_fa,yqdsAll_la);
  index_maxl.allocate(1,def_yqd,1,yqdAll,yqdsAll_fa,yqdsAll_la);
 ...
 if (multi==0) maxl=-1;
  size_l_sea.allocate(1,def_yqd,1,nsp,fa,max_a,minl,maxl);
  consum_l.allocate(1,def_yqd,1,npr,fa,max_a,minl,maxl);
 ...
 if (multi>=0) {
  ...
  l_stl_yqdplpl.allocate(1,n_stl_yqdpl);
 ...
 if (test_output==3 && multi>0) cout<<"starts reading file: stomcon_at_length.in"<<endl;
  ...
  stl_stom_input.allocate(1,n_stl_yqdpl,1,l_stl_yqdplpl,"stl_stom_input");
 if (test_output==3 && multi>0) cout<<"Relaltive stomach contents from file stomcon_at_length.in:"<<endl<<stl_stom_input<<endl;
  stl_stom.allocate(1,n_stl_yqdpl,1,l_stl_yqdplpl);
  log_stl_stom.allocate(1,n_stl_yqdpl,1,l_stl_yqdplpl);
 if (test_output==3 && multi>0) cout<<"starts reading file: stomtype_at_length.in"<<endl;
  ...
  stl_stom_type.allocate(1,n_stl_yqdpl,1,l_stl_yqdplpl,"stl_stom_type");
 if (test_output==3 && multi>0) cout<<"Type of stomach content input from file stomtype_at_length.in:"<<endl<<stl_stom_type<<endl;
  stl_stom_use_like.allocate(1,n_stl_yqdpl,1,l_stl_yqdplpl);
  stl_stom_use_avail.allocate(1,n_stl_yqdpl,1,l_stl_yqdplpl);
 if (test_output==3 && multi>0) cout<<"starts reading file: Stomlen_at_length.in"<<endl;
  ...
  stl_lstom.allocate(1,n_stl_yqdpl,1,l_stl_yqdplpl,"stl_lstom");
 ...
 if (test_output==3 && multi>0) cout<<"starts reading file: Stomnumber_at_length.in"<<endl;
  ...
  stl_nopreystom.allocate(1,n_stl_yqdpl,1,l_stl_yqdplpl,"stl_nopreystom");
 ...
 if (test_output==3 && multi>0) cout<<"starts reading file: stom_pred_length_at_sizecl.in"<<endl; 
  ...
  pred_length.allocate(1,n_stl_yqdpl,"pred_length");
 if (test_output==3 && multi>0) cout<<"Predator mean length from file stom_pred_length_at_sizecl.in:"<<endl<<pred_length<<endl;
  pred_size.allocate(1,n_stl_yqdpl);
  Prey_number_fac_term.allocate(1,n_stl_yqdpl,1,l_stl_yqdplpl);
  Prey_number_like.allocate(1,n_stl_yqdpl,1,l_stl_yqdplpl);
 Prey_number_like=0;
 ...
 if (multi==0) Mindex=-1; else Mindex=2;
  L_W_ab.allocate(1,nsp,1,2,"L_W_ab");
 ...
 if (test_output==3 && multi>0) cout<<"starts reading file: stomweight_at_length.in"<<endl;
  ...
  stl_wstom.allocate(1,n_stl_yqdpl,1,l_stl_yqdplpl,"stl_wstom");
 if (test_output==3 && multi>0) cout<<"Prey mean weight from file stomweight_at_length.in:"<<endl<<stl_wstom<<endl;
  prey_size.allocate(1,n_stl_yqdpl,1,l_stl_yqdplpl);
 int mil=100;
 int mal=0;
 ...
 if (incl_stom_all==0){
  ...
  incl_stom.allocate(1,no_areas,1,Mnpr,fq,lq,mil,mal,1,n_stl_y,"incl_stom");
  incl_stom_out.allocate(1,no_areas,1,Mnpr,fq,lq,mil,mal,1,n_stl_y);
 ...
 if (test_output==3 && multi>0) cout<<"starts reading file: n_haul_at_length.in"<<endl;
  ...
  stl_N_haul.allocate(1,n_stl_yqdpl,1,l_stl_yqdplpl,"stl_N_haul");
 if (test_output==3 && multi>0) cout<<"Number of hauls from file N_haul_at_length.in:"<<endl<<stl_N_haul<<endl;
  stl_no_samples.allocate(1,n_stl_yqdpl,1,l_stl_yqdplpl);
 if (multi>=1) {
  ...
  min_no_samples.allocate(1,Mnpr);
  max_no_samples.allocate(1,Mnpr);
  max_sumP.allocate(1,Mnpr);
 ...
 if (multi==0) Mindex=-1; else Mindex=max_a;
  other_pred_N_input.allocate(1,Mnopr,fyData,Mly,fq,Mlq,fa,Mmax_a,"other_pred_N_input");
 if (test_output==3 && multi>0 && nOthPred>0) cout<<"Other predator stock size from file other_pred_n.in:"<<endl<<other_pred_N_input<<endl;
  other_pred_N.allocate(1,yq,1,v_last_Osp,vOpr_fa,vOpr_la);
 ...
 if (multi==0) Mindex=0; else Mindex=Mnpr;
  AV_other_food.allocate(1,no_areas,1,Mnpr,"AV_other_food");
 if (test_output==3 && multi>0) cout<<"Other food biomass from file other_food.in:"<<endl<<AV_other_food<<endl;
  AV_other_food_size.allocate(1,npr);
  min_pred_length.allocate(1,npr);
  max_pred_length.allocate(1,npr);
  min_pred_prey_size_ratio.allocate(1,npr,first_VPA,nsp);
  max_pred_prey_size_ratio.allocate(1,npr,first_VPA,nsp);
  all_min_pred_prey_size_ratio.allocate(1,npr);
  all_max_pred_prey_size_ratio.allocate(1,npr);
  all_range_pred_prey_size_ratio.allocate(1,npr);
 if (test_output>=1) cout << endl << "Input of multispecies data completed" << endl;
  pred_prey_comb.allocate(1,no_areas,1,npr,first_VPA,nsp);
  max_prey_length.allocate(1,no_areas,first_VPA,nsp);
  min_prey_length.allocate(1,no_areas,first_VPA,nsp);
 ...
 CALC_yqd
 yqd_l_stom=yqd;
  stom_VPA.allocate(yqd_f_stom,yqd_l_stom);
 stom_VPA=nsp;
  stom_minl.allocate(yqd_f_stom,yqd_l_stom,first_VPA,nsp);
  stom_maxl.allocate(yqd_f_stom,yqd_l_stom,first_VPA,nsp);
 ...
 init_pop=1;
  ...
  pred_west.allocate(1,nsp,fq,lq,fa,max_a);
  pred_west_old.allocate(1,nsp,fq,lq,fa,max_a);
  pred_weca.allocate(first_VPA,nsp,fq,lq,fa,max_a);
  pred_prop_landed.allocate(first_VPA,nsp,fq,lq,fa,max_a);
  pred_propmat.allocate(first_VPA,nsp,fq,lq,fa,max_a);
  pred_size_sea.allocate(1,nsp,fq,lq,fa,max_a);
  tmp_SSB.allocate(1,ndim,1,no_MCMC_iterations,first_VPA,nsp,fyModel,lpy);
  tmp_SSB_percieved.allocate(1,ndim,1,no_MCMC_iterations,first_VPA,nsp,lyModel+1,lpy);
  tmp_TSB.allocate(1,ndim,1,no_MCMC_iterations,first_VPA,nsp,fyModel,lpy);
  tmp_mean_F.allocate(1,ndim,1,no_MCMC_iterations,first_VPA,nsp,fyModel,lpy);
  tmp_mean_F_percieved.allocate(1,ndim,1,no_MCMC_iterations,first_VPA,nsp,fyModel,lpy);
  tmp_yield.allocate(1,ndim,1,no_MCMC_iterations,first_VPA,nsp,fyModel,lpy);
  tmp_eaten_M2.allocate(1,ndim,1,no_MCMC_iterations,first_VPA,nsp,fyModel,lpy);
  tmp_recruit.allocate(1,ndim,1,no_MCMC_iterations,first_VPA,nsp,fyModel,lpy);
 sdReportYear=min(30,lyModel-fyModel);        // number of years in SD report for mean F and SSB
  environment.allocate(lyModel+1,lyModel+93,1,2);
}

void model_parameters::initializationfunction(void)
{
  F_y_ini.set_initial_value(1.0);
  F_q_ini.set_initial_value(1.0);
  log_F_a_ini.set_initial_value(0.0);
  F_y_spline.set_initial_value(1.0);
  log_rec.set_initial_value(0.0);
  log_rec_older.set_initial_value(0.0);
  SSB_R_alfa.set_initial_value(1.0);
  SSB_R_beta_ini.set_initial_value(0.5);
  qq_ini.set_initial_value(1.0);
  qq_power_ini.set_initial_value(1.0);
  init_s1.set_initial_value(10.0);
  if (global_datafile)
  {
    delete global_datafile;
    global_datafile = NULL;
 ...
 model_data(argc,argv) , function_minimizer(sz)
{
 ...
 parexp<<" par parNo species year quarter area age predator prey fleet"<<endl;
  log_rec_scale.allocate(first_VPA,nsp,"log_rec_scale");
  #ifndef NO_AD_INITIALIZE
    log_rec_scale.initialize();
  #endif
  log_rec.allocate(first_VPA,nsp,fyModel,lyModel,-15.,10.,phase_log_rec,"log_rec");
 if  (phase_log_rec>0) {
   for (s=first_VPA;s<=nsp;s++) for (y=fyModel;y<=lyModel;y++)
    {parNo++; parexp<<"log_rec "<<parNo<<" "<<s<<" "<<y<<" -1 -1 -1 -1 -1 -1"<<endl;}
 }
  log_rec_older.allocate(first_VPA,nsp,fa+1,la_like,-20.,10.,phase_log_rec_older,"log_rec_older");
 if  (phase_log_rec_older>0) {
  ...
  F_a.allocate(first_VPA,nsp,fyModel,lyModel,fa,la_VPA,"F_a");
  #ifndef NO_AD_INITIALIZE
    F_a.initialize();
  #endif
  F_y.allocate(first_VPA,nsp,fyModel,lyModel,"F_y");
  #ifndef NO_AD_INITIALIZE
    F_y.initialize();
  #endif
  F_q.allocate(first_VPA,nsp,1,max_sag,1,max_syg,fq,lq,"F_q");
  #ifndef NO_AD_INITIALIZE
    F_q.initialize();
  #endif
  F_q_last_year.allocate(first_VPA,nsp,fq,lq,fa,la_VPA,"F_q_last_year");
  #ifndef NO_AD_INITIALIZE
    F_q_last_year.initialize();
  #endif
 ...
 for (s=first_VPA;s<=nsp;s++) if (do_effort(s)==0) {F_y_species_index(s)=i; i++;}
  F_y_ini.allocate(first_VPA,nsp_Fy,fyModel+1,lly,0.01,10.0,phase_F_y_ini,"F_y_ini");
 if (phase_F_y_ini>0 ) {           
  ...
  F_y_spline.allocate(first_VPA,nsp,1,Nknots,0.01,3,phase_F_y_spline,"F_y_spline");
 ...
 for (s=first_VPA;s<=nsp;s++) {
  ...
  F_q_ini.allocate(1,no_sp_sag_syg,fcq,lcq,0.0001,3E3,seasonal_annual_catches_phase,"F_q_ini");
 if (1==2) {
  cout<<"TEST 1:"<<endl;
 ...
 if (phase_F_q_ini>0) {
  ...
  log_F_a_ini.allocate(first_VPA,nsp,cfa,las,0,no_F_y_groups,phase_log_F_a_ini,"log_F_a_ini");
 if  (phase_log_F_a_ini>0) {
  ...
  catch_s2.allocate(first_VPA,nsp,1,seasonal_combined_catch_s2,1,n_catch_s2_group,"catch_s2");
  #ifndef NO_AD_INITIALIZE
    catch_s2.initialize();
  #endif
  fleet_contribution.allocate(1,no_sp_fl,"fleet_contribution");
  #ifndef NO_AD_INITIALIZE
    fleet_contribution.initialize();
  #endif
  fleet_contribution_mean.allocate(1,no_sp_fl,"fleet_contribution_mean");
  #ifndef NO_AD_INITIALIZE
    fleet_contribution_mean.initialize();
  #endif
  no_obj_obs.allocate(1,nsp+1,1,5,"no_obj_obs");
  #ifndef NO_AD_INITIALIZE
    no_obj_obs.initialize();
  #endif
  obj_func.allocate(1,nsp,1,6,"obj_func");
  #ifndef NO_AD_INITIALIZE
    obj_func.initialize();
  #endif
  qq_ini.allocate(1,no_sp_fl,v_first_fleet_age,v_last_fleet_age_q,1E-12,1E4,phase_qq_ini,"qq_ini");
 if (phase_qq_ini>0) {
   int no_sp_fl=0;       
   for (s=first_VPA;s<=nsp;s++) for (f=1;f<=n_fleet(s);f++){ 
     no_sp_fl++;
 ...
 if (no_of_power_exponents==0) phase_qq_power_ini=-1;
  qq_power_ini.allocate(1,no_of_power_exponents,0.1,3.0,phase_qq_power_ini,"qq_power_ini");
 if (phase_qq_power_ini>0){
  ...
  qq_s2.allocate(1,no_sp_fl,1,v_n_CPUE_s2_group,"qq_s2");
  #ifndef NO_AD_INITIALIZE
    qq_s2.initialize();
  #endif
 if (est_calc_sigma(2)==0) {
  ...
  qq_s2_ini.allocate(1,dummy_no_sp_fl,1,dummy_v_n_CPUE_s2_group,CPUE_min_s2_bound,2.0,phase_qq_ini,"qq_s2_ini");
 if (phase_qq_ini>0 && est_calc_sigma(2)==0) {
  ...
  qq.allocate(first_VPA,nsp,1,n_fleet,first_fleet_age,last_fleet_age,"qq");
  #ifndef NO_AD_INITIALIZE
    qq.initialize();
  #endif
  qq_power.allocate(first_VPA,nsp,1,n_fleet,first_fleet_age,last_fleet_age,"qq_power");
  #ifndef NO_AD_INITIALIZE
    qq_power.initialize();
  #endif
  log_CPUE.allocate(1,no_sp_fl,v_first_fleet_year,v_last_fleet_year,v_first_fleet_age,v_last_fleet_age,"log_CPUE");
  #ifndef NO_AD_INITIALIZE
    log_CPUE.initialize();
  #endif
  CPUE_residuals.allocate(1,no_sp_fl,v_first_fleet_year,v_last_fleet_year,v_first_fleet_age,v_last_fleet_age,"CPUE_residuals");
  #ifndef NO_AD_INITIALIZE
    CPUE_residuals.initialize();
  #endif
  SSB_R_alfa.allocate(first_VPA,nsp,0,1.0E4,phase_SSB_R_alfa,"SSB_R_alfa");
 ...
 for (s=first_VPA;s<=nsp;s++) if (use_beta_SSB_Rec(s)==1) i=i+1;
  SSB_R_beta_ini.allocate(1,i,1.0E-5,1.0E5,phase_SSB_R_beta,"SSB_R_beta_ini");
  SSB_R_beta.allocate(first_VPA,nsp,"SSB_R_beta");
  #ifndef NO_AD_INITIALIZE
    SSB_R_beta.initialize();
  #endif
 ...
 for (s=first_VPA;s<=nsp;s++) if (SSB_Rec_model(s)==51) i=i+1;
  RecTempVar_ini.allocate(1,i,phase_SSB_R_beta,"RecTempVar_ini");
  RecTempVar.allocate(first_VPA,nsp,"RecTempVar");
  #ifndef NO_AD_INITIALIZE
    RecTempVar.initialize();
  #endif
 ...
 xx=min_catch_CV*min_catch_CV;        // same as above???
  catch_s2_ini.allocate(first_VPA,i,1,seasonal_combined_catch_s2,1,n_catch_s2_group,xx,2.0,j,"catch_s2_ini");
 if (phase_log_F_a_ini>0 && i>0)  for (s=first_VPA;s<=nsp;s++)  for (j=1;j<=seasonal_combined_catch_s2(s);j++) for (k=1;k<=n_catch_s2_group(s);k++) 
  ...
  SSB_R_s2.allocate(first_VPA,nsp,"SSB_R_s2");
  #ifndef NO_AD_INITIALIZE
    SSB_R_s2.initialize();
  #endif
 if (est_calc_sigma(3)==0) i=nsp; else i=-1; 
  SSB_R_s2_ini.allocate(first_VPA,i,min_SR_s2,2,phase_SSB_R_alfa,"SSB_R_s2_ini");
 if (phase_SSB_R_alfa>0 && est_calc_sigma(3)==0)  for (s=first_VPA;s<=nsp;s++) {parNo++; parexp<<"SSB_R_s2_ini "<<parNo<<" "<<s<<" -1 -1 -1 -1 -1 -1 -1"<<endl; }
  C_hat_annual.allocate(fyData,lyData,first_VPA,vys2,vVPA_fay,vVPA_lay,"C_hat_annual");
  #ifndef NO_AD_INITIALIZE
    C_hat_annual.initialize();
  #endif
  M2.allocate(1,yq,first_VPA,v_last_sp,vVPA_fa,vVPA_la,"M2");
  #ifndef NO_AD_INITIALIZE
    M2.initialize();
  #endif
  old_M2.allocate(1,yq,first_VPA,v_last_sp,vVPA_fa,vVPA_la,"old_M2");
  #ifndef NO_AD_INITIALIZE
    old_M2.initialize();
  #endif
  F.allocate(1,yq1,1,v_last_sp1,vAll_fa1,vAll_la1,"F");
  #ifndef NO_AD_INITIALIZE
    F.initialize();
  #endif
  Z.allocate(1,yq1,1,v_last_sp1,vAll_fa1,vAll_la1,"Z");
  #ifndef NO_AD_INITIALIZE
    Z.initialize();
  #endif
  N.allocate(1,yq2,first_VPA,v_last_sp2,vVPA_fa2,vVPA_la2,"N");
  #ifndef NO_AD_INITIALIZE
    N.initialize();
  #endif
  N_bar.allocate(1,yq1,1,v_last_sp1,vAll_fa1,vAll_la1,"N_bar");
  #ifndef NO_AD_INITIALIZE
    N_bar.initialize();
  #endif
  N_bar_stom.allocate(1,yq1,1,v_last_sp1,vAll_fa1,vAll_la1,"N_bar_stom");
  #ifndef NO_AD_INITIALIZE
    N_bar_stom.initialize();
  #endif
  C_hat.allocate(1,yq,first_VPA,v_last_sp,vVPA_fa,vVPA_la,"C_hat");
  #ifndef NO_AD_INITIALIZE
    C_hat.initialize();
  #endif
  avail_food.allocate(1,def_yqd,1,yqdPred,yqdsPred_fa,yqdsPred_la,"avail_food");
  #ifndef NO_AD_INITIALIZE
    avail_food.initialize();
  #endif
  N_l_bar_like.allocate(yqd_f_stom,yqd_l_stom,first_VPA,stom_VPA,stom_minl,stom_maxl,"N_l_bar_like");
  #ifndef NO_AD_INITIALIZE
    N_l_bar_like.initialize();
  #endif
  ALK_adjusted.allocate(1,n_ALK_yqdsa,f_ALK_yqdsal,l_ALK_yqdsal,"ALK_adjusted");
  #ifndef NO_AD_INITIALIZE
    ALK_adjusted.initialize();
  #endif
 ALK_adjusted=ALK;
  ALKS_adjusted.allocate(1,n_ALKS_yqdsa,f_ALKS_yqdsal,l_ALKS_yqdsal,"ALKS_adjusted");
  #ifndef NO_AD_INITIALIZE
    ALKS_adjusted.initialize();
  #endif
 ALKS_adjusted=ALKS_input;
  part_M2.allocate(1,npr,fa,max_a,first_VPA,nsp,fa,max_a,"part_M2");
  #ifndef NO_AD_INITIALIZE
    part_M2.initialize();
  #endif
  other_bio.allocate(1,Mnopr,fyModel,lpy,fq,lq,"other_bio");
  #ifndef NO_AD_INITIALIZE
    other_bio.initialize();
  #endif
  TSB.allocate(first_VPA,nsp,fyModel,lpy,"TSB");
  #ifndef NO_AD_INITIALIZE
    TSB.initialize();
  #endif
  SSB.allocate(first_VPA,nsp,fyModel,lpy,"SSB");
  #ifndef NO_AD_INITIALIZE
    SSB.initialize();
  #endif
  SSB_percieved.allocate(first_VPA,nsp,lyModel+1,lpy,"SSB_percieved");
  #ifndef NO_AD_INITIALIZE
    SSB_percieved.initialize();
  #endif
  CWsum.allocate(first_VPA,nsp,fyModel,lpy,"CWsum");
  #ifndef NO_AD_INITIALIZE
    CWsum.initialize();
  #endif
  CWsum_hat.allocate(first_VPA,nsp,fyModel,lpy,"CWsum_hat");
  #ifndef NO_AD_INITIALIZE
    CWsum_hat.initialize();
  #endif
  yield.allocate(first_VPA,nsp,fyModel,lpy,"yield");
  #ifndef NO_AD_INITIALIZE
    yield.initialize();
  #endif
  yield_hat.allocate(first_VPA,nsp,fyModel,lpy,"yield_hat");
  #ifndef NO_AD_INITIALIZE
    yield_hat.initialize();
  #endif
  eaten_M2.allocate(first_VPA,nsp,fyModel,lpy,"eaten_M2");
  #ifndef NO_AD_INITIALIZE
    eaten_M2.initialize();
  #endif
  Mean_F.allocate(first_VPA,nsp,fyModel,lpy,"Mean_F");
  #ifndef NO_AD_INITIALIZE
    Mean_F.initialize();
  #endif
  Mean_F_percieved.allocate(first_VPA,nsp,fyModel,lpy,"Mean_F_percieved");
  #ifndef NO_AD_INITIALIZE
    Mean_F_percieved.initialize();
  #endif
  closure.allocate(first_VPA,nsp,lyModel+1,lpy,"closure");
  #ifndef NO_AD_INITIALIZE
    closure.initialize();
  #endif
  constraints.allocate(first_VPA,nsp,lyModel+1,lpy,"constraints");
  #ifndef NO_AD_INITIALIZE
    constraints.initialize();
  #endif
  stl_E_stom.allocate(1,n_stl_yqdpl,1,l_stl_yqdplpl,"stl_E_stom");
  #ifndef NO_AD_INITIALIZE
    stl_E_stom.initialize();
  #endif
  stl_prey_avail.allocate(1,n_stl_yqdpl,1,l_stl_yqdplpl,"stl_prey_avail");
  #ifndef NO_AD_INITIALIZE
    stl_prey_avail.initialize();
  #endif
  stl_prey_avail_part.allocate(1,n_stl_yqdpl,1,l_stl_yqdplpl,"stl_prey_avail_part");
  #ifndef NO_AD_INITIALIZE
    stl_prey_avail_part.initialize();
  #endif
 stl_prey_avail=0;
 stl_prey_avail_part=0;                
  stl_E_Nstom.allocate(1,n_stl_yqdpl,1,l_stl_yqdplpl,"stl_E_Nstom");
  #ifndef NO_AD_INITIALIZE
    stl_E_Nstom.initialize();
  #endif
  stl_Nprey_avail.allocate(1,n_stl_yqdpl,1,l_stl_yqdplpl,"stl_Nprey_avail");
  #ifndef NO_AD_INITIALIZE
    stl_Nprey_avail.initialize();
  #endif
  stl_NSumprey_avail.allocate(1,n_stl_yqdpl,1,l_stl_yqdplpl,"stl_NSumprey_avail");
  #ifndef NO_AD_INITIALIZE
    stl_NSumprey_avail.initialize();
  #endif
  sum_p_Dirichlet.allocate(1,n_stl_yqdpl,"sum_p_Dirichlet");
  #ifndef NO_AD_INITIALIZE
    sum_p_Dirichlet.initialize();
  #endif
  like_Dirichlet.allocate(1,n_stl_yqdpl,"like_Dirichlet");
  #ifndef NO_AD_INITIALIZE
    like_Dirichlet.initialize();
  #endif
 if(multi==0) no_of_pred_prey_comb=0;
  vulnera.allocate(1,no_of_pred_prey_comb,1E-3,1E3,phase_vulnera,"vulnera");
 if (phase_vulnera>0) {
   for (d=1;d<=no_areas;d++) {
     for (pred=1;pred<=npr;pred++) {
       for (prey=first_VPA;prey<=nsp;prey++) {
  ...
  init_stl_other_suit_slope.allocate(1,no_size_other_food_suit,-10,10,phase_stl_other_suit_slope,"init_stl_other_suit_slope");
  stl_other_suit_slope.allocate(1,Mnpr,"stl_other_suit_slope");
  #ifndef NO_AD_INITIALIZE
    stl_other_suit_slope.initialize();
  #endif
 ...
 if(multi==0) n=0;
  init_pref_size_ratio.allocate(1,n,spl,spu,phase_pref_size_ratio2,"init_pref_size_ratio");
  pref_size_ratio.allocate(1,npr,"pref_size_ratio");
  #ifndef NO_AD_INITIALIZE
    pref_size_ratio.initialize();
  #endif
 ...
 if (multi==0 || phase_prey_size_adjustment<=0 ) nn=first_VPA_prey-1;  // single species mode or option not in use
  ...
  init_prey_size_adjustment.allocate(first_VPA_prey,nn,0.5,4,phase_prey_size_adjustment,"init_prey_size_adjustment");
  prey_size_adjustment.allocate(first_VPA,nsp,"prey_size_adjustment");
  #ifndef NO_AD_INITIALIZE
    prey_size_adjustment.initialize();
  #endif
 ...
 if (multi>0 && phase_pref_size_ratio_correction>-1)  n=npr;
  init_pref_size_ratio_correction.allocate(1,n,-5,5,phase_pref_size_ratio_correction,"init_pref_size_ratio_correction");
  pref_size_ratio_correction.allocate(1,npr,"pref_size_ratio_correction");
  #ifndef NO_AD_INITIALIZE
    pref_size_ratio_correction.initialize();
  #endif
 ...
 if(multi==0) n=0;
  var_size_ratio_ini.allocate(1,n,vpl,vpu,phase_var_size_ratio2,"var_size_ratio_ini");
 if (phase_var_size_ratio>0) {
  ...
  var_size_ratio.allocate(1,npr,"var_size_ratio");
  #ifndef NO_AD_INITIALIZE
    var_size_ratio.initialize();
  #endif
  season_overlap.allocate(1,no_areas,1,npr,fq,lq,0,nsp,"season_overlap");
  #ifndef NO_AD_INITIALIZE
    season_overlap.initialize();
  #endif
  init_season_overlap.allocate(1,no_season_overlap_to_estimate,0.05,20.0,phase_season_overlap,"init_season_overlap");
 if (phase_season_overlap>0) {
 ...
 dvector Stom_var_u(1,n);
  Stom_var_l_save.allocate(1,n,"Stom_var_l_save");
  #ifndef NO_AD_INITIALIZE
    Stom_var_l_save.initialize();
  #endif
  Stom_var_u_save.allocate(1,n,"Stom_var_u_save");
  #ifndef NO_AD_INITIALIZE
    Stom_var_u_save.initialize();
  #endif
 ...
 for (s=first_VPA;s<=nsp;s++) for (a=cfa(s);a<=la(s);a++) na_spF++;
  term_F.allocate(1,na_spF,"term_F");
  #ifndef NO_AD_INITIALIZE
    term_F.initialize();
  #endif
  exploi_pattern.allocate(fq,lq,1,na_spF,"exploi_pattern");
  #ifndef NO_AD_INITIALIZE
    exploi_pattern.initialize();
  #endif
  log_term_F.allocate(1,na_spF,"log_term_F");
  #ifndef NO_AD_INITIALIZE
    log_term_F.initialize();
  #endif
  next_SSB.allocate(first_VPA,nsp,lyModel+1,lyModel+1,"next_SSB");
  #ifndef NO_AD_INITIALIZE
    next_SSB.initialize();
  #endif
  short_term_SSB.allocate(first_VPA,nsp,1,no_F_multipliers,"short_term_SSB");
  #ifndef NO_AD_INITIALIZE
    short_term_SSB.initialize();
  #endif
 short_term_SSB=0;
  log_short_term_SSB.allocate(first_VPA,nsp,1,no_F_multipliers,"log_short_term_SSB");
  #ifndef NO_AD_INITIALIZE
    log_short_term_SSB.initialize();
  #endif
 log_short_term_SSB=0;
  SSB_likeprof.allocate("SSB_likeprof");
  #ifndef NO_AD_INITIALIZE
  SSB_likeprof.initialize();
  #endif
 SSB_likeprof=0;
  stock_N_likeprof.allocate("stock_N_likeprof");
  #ifndef NO_AD_INITIALIZE
  stock_N_likeprof.initialize();
  #endif
 stock_N_likeprof=0;
  short_term_yield.allocate(first_VPA,nsp,1,no_F_multipliers,"short_term_yield");
  #ifndef NO_AD_INITIALIZE
    short_term_yield.initialize();
  #endif
  short_term_F.allocate(first_VPA,nsp,1,no_F_multipliers,"short_term_F");
  #ifndef NO_AD_INITIALIZE
    short_term_F.initialize();
  #endif
  short_term_SSB0.allocate(first_VPA,nsp,1,no_F_multipliers,"short_term_SSB0");
  #ifndef NO_AD_INITIALIZE
    short_term_SSB0.initialize();
  #endif
  M2_sd0.allocate(1,nprey,lyModel-sdReportYear,lyModel,"M2_sd0");
  #ifndef NO_AD_INITIALIZE
    M2_sd0.initialize();
  #endif
  M2_sd1.allocate(1,nprey,fyModel,lyModel,"M2_sd1");
  #ifndef NO_AD_INITIALIZE
    M2_sd1.initialize();
  #endif
  M2_sd2.allocate(1,nprey,lyModel-sdReportYear,lyModel,"M2_sd2");
  #ifndef NO_AD_INITIALIZE
    M2_sd2.initialize();
  #endif
  rec_sd.allocate(first_VPA,nsp,lyModel-sdReportYear,lyModel,"rec_sd");
...
void model_parameters::preliminary_calculations(void)
{
 ...
 // "uncomment" if uncertanites for forecast are used
   ...
   for (y=fyModel;y<=lyModel;y++) {
     ...
     for (q=fq;q<=lqLocal;q++) {
      ...
      for (a=faq(q);a<=la(s);a++) {
        ...
        if (mceval==0 ) {
         if (( (other_pred_N(yq,s,a)>0 && consum(yqd,s,a)==0) )&& test_output>=1 )
         ...
         for (La=ALKS_yqds(Ls,2);La<=ALKS_yqds(Ls,3);La++) {                                                                   
           ...
           for (Ll=ALKS_yqdsa(La,2);Ll<=ALKS_yqdsa(La,3);Ll++) {
             ...
             if (s<=npr) {
               ...
               else consum_l(yqd,s,a,Ll)= consum_ab(d,s,q,1)* pow(size_l_sea(yqd,s,a,Ll),consum_ab(d,s,q,2));
             }
           }
         } 
 }}}}}  
 ...
 // find minimum and maximum pred prey size ratio
  ...
  min_pred_prey_size_ratio=1000.0;
  max_pred_prey_size_ratio=-1000.0;
 for (sy=1;sy<=n_stl_y;sy++) { 
   for (sq=stl_y(sy,2);sq<=stl_y(sy,3);sq++) { 
     for (sd=stl_yq(sq,2);sd<=stl_yq(sq,3);sd++) { 
       for (sp=stl_yqd(sd,2);sp<=stl_yqd(sd,3);sp++) {
  ...
  if (mceval==0) {
   cout<<endl<<"observed min predator prey size ratio: "<<endl;
    ...
    for (pred=1;pred<=npr;pred++) {
      ...
      for (prey=first_VPA;prey<=nsp;prey++) {     
        if (min_pred_prey_size_ratio(pred,prey)==1000) cout<<"          .";
 ...
 if (mceval==0) {
  ...
  all_min_pred_prey_size_ratio=1000;
  all_max_pred_prey_size_ratio=-1000;
  if (mceval==0) {
 ...
 if (mceval==0) {
  ...
  for (y=fyModel;y<=lyModel;y++) for (q=fq;q<=lq;q++) { 
    CALC_yq
    M2(yq)=M(yq)-M1(yq);  //initial guess on M2
  }  
...
void model_parameters::userfunction(void)
{
 ...
 if (sd_phase()){
  if (do_short_term_forecast==1) make_short_term_forecast_F();
...
void model_parameters::make_short_term_forecast_TAC()
{
 int s,a,y,Fm,yqr;
...
void model_parameters::make_short_term_forecast_F()
{
 int s,a,y,Fm,yqr;
 short_term_SSB=0;
 short_term_SSB0=0;
...
void model_parameters::N_at_length(int y,int q,dvar_matrix N_bar, dvar3_array& N_l_bar)
{
 int s,a;
...
void model_parameters::calc_M2_simple(int y, int q, int d, dvector other_food, dmatrix size, dmatrix prey_w, dmatrix consum, dvar_matrix N_bar, dvar_matrix& avail_food, dvar_matrix& M2)
{
     ...
     tmp=N_bar(pred,pred_a)*consum(pred,pred_a)/avail_food(pred,pred_a);
      ...
      for (prey=first_VPA;prey<=nsp;prey++) {
        if (pred_prey_comb(d,pred,prey)>0) {
          for (prey_a=faq(q);prey_a<=la(prey);prey_a++) {
            prey_size=size(prey,prey_a);
            if ((pred_size*prey_pred_size_fac(pred))>=prey_size) M2(prey,prey_a)+=tmp*suit(y,q,d,pred,prey,pred_size,prey_size,observed);
      }}}    
     }  
   }
 } 
...
void model_parameters::calc_M2_ALK(int y, int q, int d, dvector other_food, d3_array size,d3_array consum, dvar_matrix N_bar, dvar_matrix N, dvar_matrix F, dvar_matrix M1, dvar_matrix& M2)
{
 int pred,pred_a,prey,prey_a,pred_l,prey_l;
 int observed=0;   // not observed stomach contents data
 double pred_size,prey_size;
...
void model_parameters::calc_M2_one_area(int y, int q)
{
 int s,d,pred;
...
void model_parameters::calc_M2_many_areas(int y, int q)
{
 int s,d,a,pred;
 int yq;  // year quarter index
 int yqd;  // quarter division index
...
void model_parameters::calc_M2(int y, int q)
{
 if (multi>=2) {
...
void model_parameters::calc_part_M2(int y,int q)
{
 int yq, yqd;
...
dvariable model_parameters::suit(int y, int q, int d, int pred,int prey,double pred_size,double prey_size, int observed)
{
 ...
 if (size_selection(pred)==0) {         // (uniform) no size selection
  ...
  { log_ratio=log(ratio);
    //if (pred==17 && prey==20) cout<<"1:"<<size_range_a_b(1,pred,prey)<<" 2:"<<size_range_a_b(2,pred,prey)<<" 3:"<<size_range_a_b(3,pred,prey)<<" 4:"<<size_range_a_b(4,pred,prey)<<" log_ratio:"<<log_ratio<<" lower:"<<size_range_a_b(1,pred,prey)+ size_range_a_b(2,pred,prey)*log(pred_size)<<" upper:"<<size_range_a_b(3,pred,prey)+ size_range_a_b(4,pred,prey)*log(pred_size)<<endl;
...
dvariable model_parameters::other_suit(int pred,double pred_size,int y, int q, int d)
{
 dvariable tmp;
...
void model_parameters::N_at_length_like(void)
{
 int s,y,q,d,a,l,ll;
     ...
     for (sd=stl_yq(sq,2);sd<=stl_yq(sq,3);sd++) { 
      ...
      for (sp=stl_yqd(sd,2);sp<=stl_yqd(sd,3);sp++) {
       for (spl=stl_yqdp(sp,2);spl<=stl_yqdp(sp,3);spl++) {
        ...
        //cout<<"stl_N_bar(spl):"<<"y:"<<y<<" q:"<<q<<" d:"<<d<<" sp="<<sp<<" predator:"<<stl_yqdp(sp,1)<<" spl="<<spl<<" pred l:"<<stl_yqdpl(spl,1)<<endl<<stl_N_bar(spl)<<endl;
        }
      }
    }
   }
...
void model_parameters::calc_expected_stomach_content(void)
{
     ...
     for (sd=stl_yq(sq,2);sd<=stl_yq(sq,3);sd++) {
       ...
       for (sp=stl_yqd(sd,2);sp<=stl_yqd(sd,3);sp++) {
             ...
             for (iprey_l=stl_yqdplp(splp,2);iprey_l<=stl_yqdplp(splp,3);iprey_l++) {
               ...
               if (prey>0) {
                if(stl_stom_use_avail(spl,ll)==1) { 
                   ...
                   if (tmp==0) {
                        cout<<"Something is wrong !!! (prey availeble food=0.0  y:"<<y<<" q:"<<q<<" pred:"<<pred<<" prey:"<< prey<<" pred_s:"<< pred_s<<" prey_s:"<<prey_s<<" log(pred size/prey size):"<<log(pred_s/prey_s)<<endl;
         ...
         if (stl_avail_food(spl) ==0) {
           cout<<"something might be wrong, available food=0. y:"<<y<<" q:"<<q<<" pred:"<<pred<<" prey:"<< prey<<" pred_s:"<< pred_s<<endl;
         }
         ll=0;
...
void model_parameters::CPUE_parm_adm(void)
{
 int i,j;
  if (est_calc_sigma(2)==0) {
...
void model_parameters::Rec_parm_adm(void)
{
 int ri,s;
 ri=0;
 for (s=first_VPA;s<=nsp;s++) if (use_beta_SSB_Rec(s)==1) {
...
void model_parameters::predation_parm_adm(void)
{
 int pred,prey,q,i,j;
...
void model_parameters::calc_F(int do_exploitaion_pattern)
{
 int s,y,q,a;
 int yq;
...
void model_parameters::calc_Z(int y, int q)
{
 int yq;
 CALC_yq
 if ( ((current_phase()>=stom_phase) && (multi>=2))) {
...
void model_parameters::get_initial_N_at_age(void)
{
 int s,y,a,q;
 int yq;
 for (s=first_VPA;s<=nsp;s++){
...
void model_parameters::get_N_at_age(int y, int q)
{
...
void model_parameters::get_N_bar_at_age(int y,int q)
{
...
void model_parameters::get_N_bar_stom_at_age(int y,int q)
{
 int yq;
 CALC_yq
 //Calc N to be used to estimate M2
 if (use_Nbar==1) for (s=first_VPA;s<=nsp;s++) N_bar_stom(yq,s)=N_bar(yq,s);
 else for (s=first_VPA;s<=nsp;s++) N_bar_stom(yq,s)=N(yq,s);
 //cout<<"get_N_bar_stom_at_age: y="<<y<<" Q="<<q<<endl<<setprecision(0)<<N_bar_stom(yq)<<endl;
 //********************************************************************************************* 
}

void model_parameters::get_biomass(void)
{
...
void model_parameters::get_expected_catch_at_age(void)
{
 int yq,s,y,q,a;
...
void model_parameters::catchability_move(void)
{
 int s, a, f, y, sp_fl;
...
void model_parameters::evaluate_catch_contributions(void)
{
...
void model_parameters::evaluate_CPUE_contributions(void)
{
...
void model_parameters::evaluate_SSB_recruitment_contributions(void)
{
 int s, y,q;
 ...
 int no;
 double mins,epsilon;
  gamma=10; // constant from Benoit Mesnil and Marie-Joëlle Rochet, A continious hocky stick stock-recruit model for estimating MSY reference points. ICES Journal of Marine Science, 67: June 8, 2010
...
dvariable model_parameters::calc_stom_var(int pred,int spl,int ll)
{
 dvariable tmp,stom;
...
void model_parameters::cacl_like_stom_number_multinomial(void)
{
...
void model_parameters::evaluate_stomach_contributions(void)
{
...
void model_parameters::test_out_test(void)
{
 int y,q,s;
 int yq;
...
void model_parameters::move_M2_to_sdreport(void)
{
   dvariable tmp;     // forkert avg M2 rettes senere
   int s,y,a,q,i;
   int yq;
...
void model_parameters::move_recruitment_to_sdreport(void)
{
 int s,y,yq,q;
  q=recq;
  for (y=lyModel-sdReportYear;y<=lyModel;y++) {
...
void model_parameters::calc_avg_F(void)
{
 int s,y,a,q;
 int yq;
...
dvariable model_parameters::SSB_recruit_last(int s)
{
  dvariable stock;
...
void model_parameters::calc_term_F(void)
{
 int s,y,a,q,sa;
 int yq;
 dvariable tmp,deadM1,deadM2,deadM, deadC, sumZ, meanN;
...
void model_parameters::calc_term_N(void)
{
   int y,q,s,a,sa,yq;
...
void model_parameters::calc_hist_SSB(void)
{
  int y,s,a,q;
  int yq;
  hist_SSB=0.0;
...
void model_parameters::calc_eaten_M2_hist(void)
{
...
void model_parameters::out_predict_raw2(char intype[], int iter, dmatrix tmp_in)
{
 int s,y;
 char txt[40];
 char txt2[40];
...
void model_parameters::out_predict_raw3(char intype[], int y, int iter,d3_array tmp_in)
{
 int s,q,a;
 char txt[40];
 char txt2[40];
...
void model_parameters::write_other_pred()
{
  int s,y,q;
  if (multi==2) {
...
void model_parameters::pred_N_at_length(int y,int q,d3_array pred_N_bar_stom, d3_array& pred_N_l_bar )
{
 int s,a;
 int Lq,Ls,Ld,La,Ll;    //ALKS index counters
 int yq;
...
void model_parameters::calc_growth(d3_array pred_N)
{
 int s, gs, q;
 double stock_N;
 for (s=first_VPA;s<=nsp;s++) {
...
void model_parameters::calc_pred_M2(int yy, int q, d3_array& pred_M1,d3_array& pred_M2, d3_array& pred_F, d3_array& pred_N, d3_array& pred_N_bar_stom)
{
 int pred, pred_a, prey, prey_a, pred_l, prey_l;
 int y,d;
...
double model_parameters::calc_SSB_from_Fscaling( int s, double F_scaling, int first_q, dmatrix M, dmatrix M1, dmatrix M2,dmatrix F_sq, dmatrix N_obs, double rec, dmatrix west, dmatrix propmat)
{
...
double model_parameters::calc_Yield_from_Fscaling( int s, int firstQ, double F_scaling, dmatrix M, dmatrix M1, dmatrix M2, dmatrix F_sq, dmatrix N_obs, dmatrix weca, dmatrix pred_prop_landed)
{
  dmatrix    F(fq,lq,fa,max_a );
  dmatrix    Z(fq,lq,fa,max_a );
  dmatrix    N(fq,lq,fa,max_a );
...
double model_parameters::find_Fscaling_from_target_SSB(int s, int first_q, double lower, double upper, double target_SSB, dmatrix M, dmatrix M1, dmatrix M2, dmatrix pred_F, dmatrix N_obs, dmatrix west, dmatrix propmat,int year)
{
  double dif,x,y;
  double SSB0,rec1;
  int iter,a;
...
double model_parameters::find_Fscaling_from_target_SSB_two_years(int s, int first_q, double lower, double upper, double target_SSB, dmatrix M, dmatrix M1, dmatrix M2, dmatrix pred_F, dmatrix pred_F_year2, dmatrix N_obs, dmatrix west, dmatrix weca, dmatrix propmat,int year,double& yieldQ4, double& yieldQ1Q3)
{
  double dif,x,y;
  double SSB0,rec2;
  int iter,a;
...
double model_parameters::find_Fscaling_from_target_yield_two_years(int s, int firstQ, double lower, double upper, double target, dmatrix M, dmatrix M1, dmatrix M2, dmatrix pred_F, dmatrix pred_F_year2, dmatrix N_obs, dmatrix weca,dmatrix west, dmatrix propmat, dmatrix pred_prop_landed,double& yieldQ4, double& yieldQ1Q3)
{
...
double model_parameters::find_Fscaling_from_target_yield(int s, int firstQ, double lower, double upper, double target, dmatrix M, dmatrix M1, dmatrix M2, dmatrix pred_F, dmatrix N_obs, dmatrix weca, dmatrix pred_prop_landed)
{
  // find a scaling factor to mean F status quo to obtain a target yield
  double dif,x,y;
  int iter;
...
void model_parameters::get_pred_Nbar_stom_at_age(int q, d3_array pred_N, d3_array& pred_Nbar, d3_array& pred_Nbar_stom)
{
 int s,a;
  //Calc N to be used to estimate M2
  for (s=first_VPA;s<=nsp;s++){
...
double model_parameters::SSB_recruit(int s, double SSB, double noise, dmatrix hist_rec_noise,int year)
{
  double noiseFac, rec;
...
void model_parameters::estimate_recruits(int y, int first_s, int last_s, int add_noise, int true_N, d3_array& pred_N, dmatrix& recruit, dmatrix& SSB_obs, d3_array pred_west, d3_array pred_propmat, dmatrix rn, dmatrix hist_rec_noise)
{
 double noise=0.0;
 double ssb,tmp;
 int s,a;
...
double model_parameters::uncertanty(int s, dvector un, double noise)
{
  double x;
...
void model_parameters::age_uncertanty(int s, dvector un, dvector& N, dvector CV)
{
...
void model_parameters::cov_uncertanty(int s, dvector un, dvector& N, dmatrix cov)
{
...
void model_parameters::cov_uncertanty_no_rec(int s, dvector un, dvector& N, dmatrix cov)
{
...
double model_parameters::real_time_estimate(int s, int TAC_year, d3_array N_true, d3_array Z, double noise)
{
 double N1;
...
double model_parameters::survey_estimate_last_year(int s, int q, d3_array N,  double noise)
{
 double index;
 // estimate observed N(year, first season, first age+1) from true N(year-1,last season, first age)
 //   and real-time uncertanties
...
double model_parameters::do_HCR_trigger(int s,double x)
{
 double tmp;
...
void model_parameters::predict_year(int y, int first_sp, int last_sp, int do_M2, int do_catch, int new_year, d3_array& pred_N, d3_array& pred_N_other, d3_array pred_F, d3_array& pred_C, d3_array pred_M, d3_array pred_M1, d3_array& pred_M2, d3_array& pred_Z)
{
...
void model_parameters::predict(void)
{
...
void model_parameters::make_sim_data_proc(int iteration)
{
  int s,y,a,q, no_set;
...
void model_parameters::do_predict(int MCMC_iteration)
{
...
void model_parameters::Calc_avg_M2(void)
{
 int s,y,q,a,no;
 dvariable tmp;
 if (multi>=2) {
...
void model_parameters::write_warnings(void)
{
...
void model_parameters::check_print_par(void)
{
 int s;
...
void model_parameters::print_input_par(void)
{
  int s,fl;
  cout << endl;
...
void model_parameters::print_summary_MCMC()
{
 int s,y,q,a;
 int yq;
...
void model_parameters::print_summary()
{
 int s,y,q,a;
 int yq;
 int d;
 ...
 res <<"Year Quarter Species.n Age M1 M2 M F Z N N.bar C.hat C.obs west weca Yield CWsum prop.landed propmat BIO SSB Lsea ration"<<endl;
  for (s=first_VPA;s<=nsp;s++){
    for (y=fyModel;y<=lyModel;y++){
      lqLocal=(y==lyModel)?lqly:lq; 
      for (q=fq;q<=lqLocal;q++){
      ...
      for (q=fq;q<=lqLocal;q++){
        ...
        for (a=fa;a<=la(s);a++) if (!(a==fa && q <recq)) {
           if ( N(yq,s,a)>0) {
             ...
             res <<"-1 -1 ";  // C_hat obs_C
             res <<west(yq,s,a)<<" ";
             res <<"-1 -1 -1 -1 -1 "; // weca yield CWsum prop.landed propmat
             res <<N(yq,s,a)*west(yq,s,a)<<" ";
             res <<"-1 ";  // SSB
             res <<lsea(yqd,s,a)<<" "; 
             res <<consum(yqd,s,a)<<" "; 
  ...
  // Year Quarter Species.n Age M1 M2 M F Z N N.bar C.hat C.obs west weca Yield CWsum prop.landed propmat BIO SSB Lsea ration
    ...
    CALC_yqd
        for (a=faq(1);a<=la(s);a++) {
           ...
           res <<west(yq-lq,s,a)<<" ";
           res <<" -1 ";        // weca
           res <<"-1 -1 -1 ";        // yield CWsum prop.landed
...
void model_parameters::print_summary_areas()
{
...
void model_parameters::print_summary_table()
{
 int s,y,q,a;
 dvariable tmp;
 int yq;
 ...
 for (s=first_VPA;s<=nsp;s++){
    ...
    if (fq==lq || lqly==lq) {  // annual assessment, calch SSB the year after the last assess. year
      ...
      CALC_yq
      for (a=fa+1;a<=la(s);a++){
         tmp+=N((lyModel+1-fyModel)*lq+fq,s,a)*west(yq,s,a)*propmat(yq,s,a)*exp(-(prop_M(s)*M(yq,s,a)+prop_F(s)*F(yq,s,a))) ;
      }
      res<<lyModel+1<<"  ";
 ...
 for (s=first_VPA;s<=nsp;s++){
    ...
    if (fq==lq || lqly==lq) {  // annual assessment, calch SSB the year after the last assess. year
     ...
     q=fq;
     CALC_yq
     for (a=fa+1;a<=la(s);a++) tmp+=N(yq+lq,s,a)*west(yq,s,a)*propmat(yq,s,a)*exp(-(prop_M(s)*M(yq,s,a)+prop_F(s)*F(yq,s,a)));
...
void model_parameters::print_rec_scale()
{
 int s;
...
void model_parameters::print_vulnerab()
{
 int pred,prey,i,d;
...
void model_parameters::print_summary_stom()
{
...
void model_parameters::print_survey_residuals()
{
 int y,s,q,a,sp_fl,f;
...
void model_parameters::print_part_M2()
{
   ...
   for (q=fq;q<=lq;q++){
     ...
     for (pred=1;pred<=npr;pred++) {
       for (pred_a=fa;pred_a<=la(pred);pred_a++) {
         for (prey=first_VPA;prey<=nsp;prey++){
           for (prey_a=fa;prey_a<=la(prey);prey_a++) {
...
void model_parameters::print_ALKS()
{
 int s,y,q,d,a;
 int Ly,Lq,Ld,Ls,La,Ll;    
 int yq;
...
void model_parameters::print_ALK()
{
 int s,y,q,d,a;
 int Ly,Lq,Ld,Ls,La,Ll;    
 int yq;
...
void model_parameters::print_catch_residuals()
{
 int y,s,q,a;
 int yq;
...
void model_parameters::print_catch_survey_residuals()
{
...
void model_parameters::print_size_pref()
{
 int s;
...
void model_parameters::print_min_max_size_pref()
{
   int s,pred,prey;
   double x;
...
void model_parameters::print_vars(adstring txt,adstring intype)
{
 int y,s,q,a;
 int yq;
...
void model_parameters::print_vars_ICES(adstring txt,adstring intype)
{
...
void model_parameters::Print_OP_seed()
{
...
void model_parameters::print_Config_parms()
{
 int i,s,d,pred,prey;
 ...
 if (multi==2) {
   ...
   op<<consum_op<<"   # Usage of food-rations from input values or from size and regression parameters (option consum)"<<endl;
...
void model_parameters::out_OP_growth()
{
...
void model_parameters::out_OP(char fname[],char text1[],int fsp,int lsp, int precis,d3_array v, imatrix yrange, int nd)
{
 int s,y,q,d;
 char txt[40];
 char txt2[40];
...
void model_parameters::print_Operating_input()
{
...
void model_parameters::print_SSB_R()
{
 int s;
...
void model_parameters::print_F_status_quo()
{
 int s,y,q;
 y=lyModel;
...
void model_parameters::print_M1M2sum()
{
 int y,s,q,a;
 int yq;
...
void model_parameters::print_season_overlap()
{
 int pred,prey,q;
...
int model_parameters::npar_matrix( dmatrix ma)
{
...
int model_parameters::npar_vector( dvector ma)
{
 // count number of active parameters in a variable
 return ma.indexmax()-ma.indexmin()+1;
}

void model_parameters::out_predict_mean(char intype[],int kind, d4_array tmp_in)
{
...
void model_parameters::write_predict_output()
{
  char txt [20];
...
void model_parameters::print_objective_func_file()
{
 int s;
...
void model_parameters::print_survey_data_noise(int no_set,int methode)
{
...
void model_parameters::print_survey_data_sim(int no)
{
...
void model_parameters::print_stomach_data_sim(int no)
{
 char txt[40];
 char txt2[10];
...
void model_parameters::print_catch_data_noise(int no_set,int methode)
{
...
void model_parameters::print_catch_data_sim(int no)
{
  int s, y, q, a;
  char txt[40];
  char txt2[10];
...
void model_parameters::report(const dvector& gradients)
{
 ...
 time (&end_time);
 report<<"Date: "<<dateStr<<"  Start time:"<<f_timeStr<<" run time:"<<difftime(end_time,start_time)<<" seconds"<<endl;
 ...
 for (s=first_VPA;s<=nsp;s++){
   ...
   if (SSB_Rec_model(s)==1)        report<<" Ricker:               ";
